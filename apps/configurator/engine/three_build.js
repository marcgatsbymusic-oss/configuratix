...
// ---------- hollow body + EXT/INT bi-color ----------
// Per the mobile-perf requirement: extrude the OUTER silhouette only (no chambers).
// Per bi-color: split that outline at a depth `seamY` into exterior + interior
// halves, each extruded separately with its own material. Works for frame, sash,
// mullion, transom — any swept profile.
function clipHalfPlane(pts, seamY, keepBelow) {
  // Sutherland-Hodgman against the horizontal line y = seamY.
  const inside = (p) => (keepBelow ? p[1] <= seamY : p[1] >= seamY);
  const intersect = (a, b) => {
    const t = (seamY - a[1]) / (b[1] - a[1]);
    return [a[0] + t * (b[0] - a[0]), seamY];
  };
  const out = [];
  for (let i = 0; i < pts.length; i++) {
...
// Returns the outer silhouette polygon (largest closed loop), CCW.
export function outerSilhouette(loops) {
  const { outer } = classifyLoops(loops);
  return outer < 0 ? null : orient(loops[outer].pts, true);
}

// Build a hollow, bi-color member. seamY is the depth coordinate where EXT meets
// INT (default: mid-depth). Returns a THREE.Group with ext + int extrusions.
export function buildHollowBicolor(loops, length, THREE, extMat, intMat, opts = {}) {
  const outer = outerSilhouette(loops);
  if (!outer) return null;
  const ys = outer.map(p => p[1]);
  const seamY = opts.seamY ?? (Math.min(...ys) + Math.max(...ys)) / 2;
  ...
  make(clipHalfPlane(outer, seamY, true), extMat);   // exterior half (y <= seam)
  make(clipHalfPlane(outer, seamY, false), intMat);  // interior half (y >= seam)
  return group;
}
