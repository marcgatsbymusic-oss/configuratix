...
function buildF252(THREE, PROFILES, HANDLE, opts) {
  ...
  const TR_GLZ   = TAXIS - 26;         // fixed glazing top edge, up into the transom rebate
  ...
  const M = Object.assign({
    pvc_ext:  new THREE.MeshStandardMaterial({ color: 0xeef0f3, roughness: .60, metalness: .03, side: S }),
    pvc_int:  new THREE.MeshStandardMaterial({ color: 0xeef0f3, roughness: .60, metalness: .03, side: S }),
  ...
  const frameGroup = new THREE.Group(); frameGroup.name = "frameGroup";
  const sashGroup = new THREE.Group(); sashGroup.name = "sashGroup";
  root.add(frameGroup);
  root.add(sashGroup);
  ...
  function box(w, h, d, cx, cy, cz, mk) {
    ...
    g.position.set(cx, cy, cz); currentParent.add(g); parts.push({ mesh: g, role: mk }); return g;
  }
  ...
  // ---- ASSEMBLY ----------------------------------------------------------
  currentParent = frameGroup;
  // FRAME — 4 mitred sides (revOuter = 0). Same profile all round (rama 01).
  [['FRM_EXT','pvc_ext'],['FRM_INT','pvc_int'],['GSK_FRM','gasket']].forEach(([lp, mk]) => {
  ...
  [['SSH_EXT','pvc_ext'],['SSH_INT','pvc_int'],['GSK_SSH_EXT','gasket'],['GSK_SSH_INT','gasket']].forEach(([lp, mk]) => {
  ...
  [['POST_EXT','pvc_ext'],['POST_INT','pvc_int'],['GSK_POST_A','gasket'],['GSK_POST_B','gasket']].forEach(([lp, mk]) => {
    buildMember(lp, r => TIN, r => W - TIN, Z, Yp, Xp, [0, TAXIS - 65, 0], mk); // reveal centre 65 -> worldY offset TAXIS - 65
  });

  // SASH GLAZING BEAD + BEAD GASKET (mitred rings)
  mitreRing('BZD_SSH',     90,  sashBot + 52, W - 90,  H - 90,  90,  'pvc_int');
  ...
  uRing('BZD_FRM',     40, 40, W - 40, 40, TR_GLZ,     'pvc_int');
  uRing('GSK_BZD_FRM', 56, 56, W - 56, 56, TR_GLZ - 6, 'gasket');
  buildMember('BZD_POST',     r => TIN, r => W - TIN, Z, Yp, Xp, [0, TAXIS - 65, 0], 'pvc_int');  // transom fixed-side bead
  buildMember('GSK_BZD_POST', r => TIN, r => W - TIN, Z, Yp, Xp, [0, TAXIS - 65, 0], 'gasket'); // transom fixed-side bead gasket

  currentParent = sashGroup;
  ...
  if (HANDLE) {
    ...
    currentParent.add(h); parts.push({ mesh: h, role: 'hardware' });
  }

  root.userData = { parts, sashGroup, frameGroup, PARAMS: { W, H, DEPTH, TAXIS, TFACE, SM, LAP, TIN, TR_GLZ, tTop, tBot, sashBot } };
  return root;
}
