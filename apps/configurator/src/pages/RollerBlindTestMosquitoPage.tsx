/**
 * RollerBlindTestMosquitoPage.tsx
 * Standalone page for testing/previewing the ROLLER_BLIND_BOX_225 roller blinds with mosquito net.
 * Route: /roller-blind-test-mosquito
 */

import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { ROLLER_BLIND_BOX_225_MosquitoViewer } from '../components/configurator/ROLLER_BLIND_BOX_225_MosquitoViewer';
import { ArrowLeft } from 'lucide-react';

export const RollerBlindTestMosquitoPage: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 flex bg-[#0e0e1a] overflow-hidden">
      ...
      <div className="relative flex-1">
        <ROLLER_BLIND_BOX_225_MosquitoViewer
          width={width}
          height={height}
        ...
        <button 
          ...
          className="absolute top-3 left-3 z-30 flex items-center justify-center p-2 rounded-lg border border-white/10 bg-[#080816]/80 hover:bg-white/5 hover:border-white/20 text-white/70 hover:text-white transition-all active:scale-95 backdrop-blur-md cursor-pointer"
      ...
      >
        <div>
          ...
          <div className="text-xs text-white/40 mt-0.5">With Integrated Mosquito Net</div>
        </div>
          ...
          <div className="flex flex-col gap-1 max-h-[120px] overflow-y-auto pr-1">
            {colors.map((col, i) => (
              <button
                ...
                className="flex items-center gap-2 px-2 py-1 rounded-lg text-left transition-all w-full cursor-pointer"
                style={{
        ...
        <div className="mt-auto pt-4 border-t border-white/5">
          <div className="text-[9px] text-white/25 leading-relaxed">
            Integrated insect protection net<br />positioned in front of the blinds
          </div>
        </div>
      </div>
    </div>
