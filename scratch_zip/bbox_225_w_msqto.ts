...
export function buildBBox225WMsqto(opts: BBoxOptions = {}): BBoxHandle {
  ...
  // ---- deploy state (scale about y=0 = box bottom) ----
  const setBlind = (t: number) => { blindGrp.scale.y = Math.max(0.001, t); };
  const setMosquito = (t: number) => { netGrp.scale.y = Math.max(0.001, t); };
  setBlind(opts.blindDeployed === false ? 0.045 : 1);
  setMosquito(opts.mosquitoDeployed === false ? 0.045 : 1);
