...
const segmentEuler = new THREE.Euler(0, 0, Math.PI, 'XYZ');
const qSegment = new THREE.Quaternion().setFromEuler(segmentEuler);
const mSegment = new THREE.Matrix4().makeRotationFromQuaternion(qSegment);

// Combined transform: outer * inner * segment
const combined = new THREE.Matrix4()
  .multiplyMatrices(mOuter, mInner)
  .multiply(mSegment);

console.log("Combined matrix row-by-row:");
