...
for (const line of lines) {
  ...
  if (line.includes('animStateRef') || line.includes('currentLeft') || line.includes('currentRight')) {
    try {
      const data = JSON.parse(line);
      console.log(`Step ${data.step_index}:`);
      const idx = line.indexOf('animStateRef');
      if (idx !== -1) {
        console.log(`  Snippet: ...${line.substring(idx - 100, idx + 200)}...`);
      } else {
        const idx2 = line.indexOf('currentLeft');
