...
function main() {
  ...
  for (const [layerName, items] of Object.entries(rawGeoms)) {
    if (!['IGE_SSH', 'IGE_MOVABLE_POST'].includes(layerName)) continue;
    const segments = [];
    ...
    stitched.forEach(chain => {
      ...
      if (closed.length > 2) contours.push(closed);
    });
    if (contours.length > 0) {
      layerContours[layerName] = contours;
    }
  }

  // Find largest closed contours
  ...
  console.log(`  Sash: area=${maxSshArea.toFixed(4)}`);
  console.log(`  Movable Post: area=${maxMpArea.toFixed(4)}`);

  console.log(`\nTesting Sash Y split points:`);
  for (let y = 30; y <= 90; y += 5) {
    const ext = clipPolygonY(largestSsh, y, true);
    const _int = clipPolygonY(largestSsh, y, false);
    const sumArea = getPolygonArea(ext) + getPolygonArea(_int);
    const diff = Math.abs(sumArea - maxSshArea);
    console.log(`  Y = ${y}: extPts=${ext.length} intPts=${_int.length} sumArea=${sumArea.toFixed(2)} diff=${diff.toFixed(6)}`);
  }

  console.log(`\nTesting Movable Post Y split points:`);
  for (let y = 45; y <= 110; y += 5) {
    const ext = clipPolygonY(largestMp, y, true);
    const _int = clipPolygonY(largestMp, y, false);
    const sumArea = getPolygonArea(ext) + getPolygonArea(_int);
    const diff = Math.abs(sumArea - maxMpArea);
    console.log(`  Y = ${y}: extPts=${ext.length} intPts=${_int.length} sumArea=${sumArea.toFixed(2)} diff=${diff.toFixed(6)}`);
  }
}
