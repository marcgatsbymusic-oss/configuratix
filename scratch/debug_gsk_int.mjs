...
function main() {
  ...
  for (const layer of ['Door_GSK_EXT', 'Door_GSK_BZD', 'Door_GSK_INT', 'Main_GSK_CENTRAL']) {
    const items = rawGeoms[layer];
    if (!items) {
    ...
    console.log(`Layer ${layer}: count=${items.length} X: [${minX.toFixed(2)}, ${maxX.toFixed(2)}] Y: [${minY.toFixed(2)}, ${maxY.toFixed(2)}]`);
  }
}
