...
console.log("Dumping all leaf polylines and lines recursively with absolute coordinates:");
...
console.log(`Collected ${items.length} leaf polylines/lines.`);
items.forEach((item, idx) => {
  console.log(`\nItem #${idx}: type=${item.type}, layer="${item.layer}", colorIndex=${item.colorIndex}`);
  console.log(`  Path: ${item.path}`);
  console.log(`  BBox: min=(${item.bbox.minX.toFixed(2)}, ${item.bbox.minY.toFixed(2)}), max=(${item.bbox.maxX.toFixed(2)}, ${item.bbox.maxY.toFixed(2)}), size=(${item.bbox.w.toFixed(2)} x ${item.bbox.h.toFixed(2)})`);
  console.log(`  Vertices: ${item.vertices.length}`);
});
