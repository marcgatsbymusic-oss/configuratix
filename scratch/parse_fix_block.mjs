 ...
 * parse_fix_block.mjs
 * ─────────────────────────────────────────────────────────────────────────────
 * DXF Pre-processor for the IGLO Edge Slide Fixed Block Part
 * File: "IGLO_EDGE_SLIDE_BLOCK_FIX_PART.dxf"
 *
 * Usage:
 *   node scratch/parse_fix_block.mjs [--out output.json]
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ─── Layer → group mapping ───────────────────────────────────────────────────
// The fixed block part is a static, non-moving element (Child2)
// Note: Profil stal and Aluminium Rail blocks in this DXF are placed at extreme
// coordinates (~5000mm away from origin) and cannot be used for cross-sections.
// Aluminium Rail from the MAIN SLE201 DXF is already included in the base profile.
const LAYER_GROUP_MAP = {
  'BLOCK_FIX_INT':       'Child2',   // Interior PVC profile of fixed frame
  'BLOCK_FIX_EXT':       'Child2',   // Exterior PVC profile of fixed frame
  'BZD':                 'Child2',   // Glazing bead on fixed glass
  'GLS_EXT':             'Child2',   // Glass (exterior pane)
  'GLS_INT':             'Child2',   // Glass (interior pane)
  'GLS_MDL':             'Child2',   // Glass (middle pane)
  'GSK_BLOCK_FIX_EXT':  'Child2',   // External gasket on fixed block
  'GSK_FIX_INT':         'Child2',   // Internal gasket on fixed block
  'GSK_BZD':             'Child2',   // Glazing bead gasket
  'SPACER':              'Child2',   // Spacer bar
};

const TARGET_LAYERS = new Set(Object.keys(LAYER_GROUP_MAP));

// Maximum allowed coordinate value for any contour centroid.
// Contours whose centroid exceeds this distance from origin in any axis are considered
// misplaced DXF blocks (e.g. aluminium rail, spacer blocks placed far from drawing origin)
// and will be discarded.
const MAX_COORD_THRESHOLD = 500;
...
async function main() {
  ...
  console.log(`\n📂 Parsing Fixed Block DXF: ${INPUT_FILE.split('/').pop()}`);
    ...
    entities.forEach(ent => {
      if (ent.type === 'INSERT') {
        ...
        if (ent.type === 'LWPOLYLINE' || ent.type === 'POLYLINE') {
          ...
          const sW = transformPoint(ent.vertices[0], tx), eW = transformPoint(ent.vertices[1], tx);
          layerSegments[matchedLayer].push({ start: sW, end: eW, pts: [sW, eW] });
  ...
  for (const layer of TARGET_LAYERS) {
    ...
    for (const chain of chains) {
      const pts = closeAndSnap(chain);
      if (pts.length > 2) {
        const simplified = simplifyContour(pts, SIMPLIFY_TOLERANCE);
        if (simplified.length > 2) {
          // Filter out contours far from cross-section origin (misplaced DXF blocks)
          const centX = simplified.reduce((s, p) => s + p.x, 0) / simplified.length;
          const centY = simplified.reduce((s, p) => s + p.y, 0) / simplified.length;
          if (Math.abs(centX) < MAX_COORD_THRESHOLD && Math.abs(centY) < MAX_COORD_THRESHOLD) {
            closedContours.push(simplified);
          } else {
            console.log(`   [SKIP] ${layer} contour at centroid (${centX.toFixed(0)}, ${centY.toFixed(0)}) — too far from origin`);
          }
        }
      }
    }
    if (closedContours.length > 0) {
      ...
      console.log(`   ${layer.padEnd(30)} → ${closedContours.length} contour(s), ${closedContours.reduce((s, c) => s + c.length, 0)} vertices`);
    }
  }
  ...
  const defaultOut = outFile || 'src/data/profiles/IgloEdge/SLE201_FixBlock.json';
