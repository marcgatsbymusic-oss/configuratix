...
function main() {
  ...
  const segments = [];
  const lwpolys = [];
  rawGeoms['Door_GSK_INT'].forEach(({ entity, tx }) => {
    if (entity.type === 'LINE') {
  ...
  const closed = closeAndSnap(stitched[0]);

  // Simplify to 10 points to see the main shape
  ...
  console.log(`Original bounding box: X=[${minX.toFixed(2)}, ${maxX.toFixed(2)}], Y=[${minY.toFixed(2)}, ${maxY.toFixed(2)}]`);
  
  // Find points at extremes
  console.log("\nExtreme points:");
  const leftPoint = closed.reduce((best, p) => p.x < best.x ? p : best, closed[0]);
  const rightPoint = closed.reduce((best, p) => p.x > best.x ? p : best, closed[0]);
  const topPoint = closed.reduce((best, p) => p.y > best.y ? p : best, closed[0]);
  const bottomPoint = closed.reduce((best, p) => p.y < best.y ? p : best, closed[0]);
  
  console.log(`Leftmost: (${leftPoint.x.toFixed(2)}, ${leftPoint.y.toFixed(2)})`);
  console.log(`Rightmost: (${rightPoint.x.toFixed(2)}, ${rightPoint.y.toFixed(2)})`);
  console.log(`Topmost: (${topPoint.x.toFixed(2)}, ${topPoint.y.toFixed(2)})`);
  console.log(`Bottommost: (${bottomPoint.x.toFixed(2)}, ${bottomPoint.y.toFixed(2)})`);
}
