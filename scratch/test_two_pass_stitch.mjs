...
// Group segments by raw X coordinate boundaries
const gsk1 = []; // left-of-post (raw X: 102 to 113)
const gsk2 = []; // right-of-post (raw X: 171 to 182)
const gsk3 = []; // sash-to-glass (raw X: 250 to 261)
...
function runTest(name, segs) {
  console.log(`\n==================================================`);
  console.log(`Testing: ${name} (segments: ${segs.length})`);
  console.log(`==================================================`);
  
  // Pass 1: Chain at 0.1mm tolerance
  const initialChains = chainSegments(segs, 0.1);
  console.log(`Initial chains at 0.1mm: ${initialChains.length}`);

  // Pass 2: Stitch open endpoints up to 1.5mm
  const stitched = stitchChains(initialChains, 1.5);
  ...
  stitched.forEach((chain, idx) => {
    ...
    const isClosed = gap <= 0.1;
    if (isClosed) {
      const area = getPolygonArea(chain);
      const selfIntersections = findSelfIntersections(chain);
      closedProfiles.push({ idx, pts: chain.length, area, intersections: selfIntersections.length, ptsList: chain });
    }
  });

  closedProfiles.sort((a, b) => b.area - a.area);
  console.log(`Closed loops (sorted by area descending):`);
  closedProfiles.forEach((p, idx) => {
    console.log(`  Loop ${idx} (original chain index ${p.idx}): pts=${p.pts}, area=${p.area.toFixed(2)} mm², intersections=${p.intersections}`);
  });
}

runTest("Gasket 1 (Left of post)", gsk1);
runTest("Gasket 2 (Right of post)", gsk2);
runTest("Gasket 3 (Sash glass)", gsk3);
