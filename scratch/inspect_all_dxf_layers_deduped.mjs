...
for (const [layerName, items] of Object.entries(rawGeoms)) {
  ...
  const solidContours = closedContours.filter(c => getPolygonArea(c) >= 1.0);
  if (solidContours.length > 0) {
    console.log(`Layer ${layerName.padEnd(25)}: solid contours found = ${solidContours.length}`);
    solidContours.forEach((c, idx) => {
      const b = getPolygonBounds(c);
      console.log(`  Contour ${idx}: area=${getPolygonArea(c).toFixed(2)}mm² X=[${b.minX.toFixed(2)}, ${b.maxX.toFixed(2)}] Y=[${b.minY.toFixed(2)}, ${b.maxY.toFixed(2)}]`);
    });
  }
}
