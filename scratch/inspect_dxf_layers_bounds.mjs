...
function main() {
  ...
  const targetLayers = ['IGE_BZD', 'IGE_GSK_MD_SSH'];
  targetLayers.forEach(layerName => {
    console.log(`\nContours for layer: ${layerName}`);
    ...
    console.log(`  Total contours found: ${contours.length}`);
    contours.forEach((c, idx) => {
      ...
      console.log(`    Contour ${idx}: pts=${c.length} area=${area.toFixed(2)} X: [${minX.toFixed(2)}, ${maxX.toFixed(2)}] Y: [${minY.toFixed(2)}, ${maxY.toFixed(2)}]`);
    });
  });
}
