...
try {
  ...
  for (const [layerName, items] of Object.entries(rawGeoms)) {
    ...
    const layerClosedContours = [];
    
    stitched.forEach(chain => {
      const closed = closeAndSnap(chain);
      if (closed.length > 2) layerClosedContours.push(closed);
    });
    ...
    if (layerClosedContours.length === 0) continue;

    console.log(`Layer "${layerName}" has ${layerClosedContours.length} closed contours.`);
    
    // Find the largest contour by area
    let largest = null, maxArea = -1;
    layerClosedContours.forEach(contour => {
      const a = getPolygonArea(contour);
      if (a > maxArea) { maxArea = a; largest = contour; }
    });
    
    if (largest) {
      const b = getPolygonBounds(largest);
      console.log(`  Largest contour area: ${maxArea.toFixed(2)} mm²`);
      console.log(`  Largest contour bounds: X [${b.minX.toFixed(2)}, ${b.maxX.toFixed(2)}]  Y [${b.minY.toFixed(2)}, ${b.maxY.toFixed(2)}]`);
    }
  }
