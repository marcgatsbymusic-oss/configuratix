...
function countIntersections(polygon, splitX) {
  let count = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Check if the edge (p1, p2) crosses splitX
...
try {
  ...
  console.log('\n--- INTERSECTION COUNT CHECK ---');
  for (let sx = 170; sx <= 200; sx += 2) {
    console.log(`Split X = ${sx}:`);
    console.log(`  SSH crossings: ${countIntersections(largestSsh, sx)}`);
    console.log(`  Movable Post crossings: ${countIntersections(largestMp, sx)}`);
  }
