...
function main() {
  ...
  console.log("Searching with smarter pocket constraint...");
  
  // We sweep dx in [8953, 8959], dy in [21288, 21295] with 0.1 step
  const step = 0.1;
  const results = [];
  ...
  for (let dx = 8953; dx <= 8959; dx += step) {
    for (let dy = 21288; dy <= 21295; dy += step) {
      ...
      translatedGsk.forEach(p => {
        if (p.x < 10.5) {
          footPoints++;
          // Must fit deep pocket
          if (p.x < 7.0 || p.y < 33.0 || p.y > 39.0) {
            fit = false;
          }
        } else if (p.x < 12.0) {
          // Must fit pocket neck
          if (p.y < 34.2 || p.y > 38.0) {
            fit = false;
          }
        }
