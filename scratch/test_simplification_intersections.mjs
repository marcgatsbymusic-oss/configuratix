...
function closeAndSnap(pts, tol = 1.5) {
...
const dxf = parser.parseSync(fs.readFileSync(INPUT_FILE, 'utf8'));
const rawGeoms = [];
...
const candidateSegs = segments.filter(seg => {
  const minX = Math.min(seg.start.x, seg.end.x);
  return minX < 25.0;
});

const initialChains = chainSegments(candidateSegs, 0.1);
const stitched = stitchChains(initialChains, 1.5);

stitched.forEach(chain => {
  const closed = [...chain];
  const rawGap = dist(closed[0], closed[closed.length - 1]);
  console.log(`\nFiltered Stitched with Snap-to-First: pts=${closed.length}, rawGap=${rawGap.toFixed(6)} mm`);
  
  const tols = [0.0, 0.001, 0.005, 0.01, 0.02, 0.05];
  tols.forEach(tol => {
    let testPts = [...closed];
    testPts = closeAndSnap(testPts, 1.5);
    
    if (tol > 0) {
      testPts = simplifyContour(testPts, tol);
    }
    const selfInter = findSelfIntersections(testPts);
    const finalGap = dist(testPts[0], testPts[testPts.length - 1]);
    console.log(`  Simplify Tol ${tol.toFixed(3)}mm: pts=${testPts.length}, finalGap=${finalGap.toFixed(6)} mm, self-intersections=${selfInter.length}`);
  });
});
