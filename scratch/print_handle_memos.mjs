...
for (const line of lines) {
  ...
  if (line.includes('clonedHandleLeft') || line.includes('clonedHandleRight')) {
    try {
      const data = JSON.parse(line);
      console.log(`Step ${data.step_index}:`);
      const idx = line.indexOf('clonedHandleLeft');
      console.log(`  Snippet: ...${line.substring(Math.max(0, idx - 100), Math.min(line.length, idx + 800))}...`);
    } catch (e) {
