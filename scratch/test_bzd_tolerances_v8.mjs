...
const items = rawGeoms['IGE_GSK_BZD'] || [];
const segments = [];
...
tols.forEach(tol => {
  ...
  // Filter right side (raw X >= 140)
  const rightContours = closedContours.filter(c => {
    let minX = Infinity;
  ...
  const sorted = rightContours.map(c => ({
    points: c,
    area: getPolygonArea(c),
    intersections: findSelfIntersections(c).length
  })).sort((a, b) => b.area - a.area);

  console.log(`\nStitch tolerance ${tol}mm:`);
  sorted.slice(0, 3).forEach((item, idx) => {
    console.log(`  Loop ${idx}: pts=${item.points.length}, area=${item.area.toFixed(2)} mm², intersections=${item.intersections}`);
  });
});
