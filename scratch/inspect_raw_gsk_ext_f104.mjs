...
// Filter only the segments with avgX < 25.0
const filteredSegs = segments.filter(s => {
  const avgX = (s.start.x + s.end.x) / 2;
  return avgX < 25.0;
});

console.log(`Total frame external segments (X < 25.0): ${filteredSegs.length}`);

// Test stitching with different tolerances
[0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 0.8, 1.0, 1.5].forEach(tol => {
  const initialChains = chainSegments(filteredSegs, 0.1);
  const stitched = stitchChains(initialChains, tol);
  console.log(`\nStitch tolerance = ${tol}mm:`);
  console.log(`  Chains count = ${stitched.length}`);
  stitched.forEach((c, idx) => {
    const gap = dist(c.pts[0], c.pts[c.pts.length - 1]);
    console.log(`    Chain ${idx}: pts=${c.pts.length}, gap=${gap.toFixed(6)}mm, closed=${c.closed}`);
  });
});
