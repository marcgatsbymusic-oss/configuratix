...
function main() {
  ...
  console.log("Searching for the perfect fit inside the pocket...");
  
  // We sweep dx in [8950, 8965], dy in [21285, 21305]
  // We want to find a translation where:
  // 1. The foot of the gasket (X_world < 12.0) fits inside X: [7.0, 12.0], Y: [33.0, 39.0].
  // 2. The number of points of the gasket inside the solid sash profile (isPointInPolygon) is minimized.
  // 3. The minimum distance of the gasket foot to the pocket interior surface is minimized.
  
  const step = 0.05;
  const results = [];
  
  for (let dx = 8950; dx <= 8965; dx += step) {
    for (let dy = 21285; dy <= 21305; dy += step) {
      const translatedGsk = misplacedGsk.map(p => ({ x: p.x + dx, y: p.y + dy }));
      
      // Check if foot is inside the pocket bounding box
      let footInPocket = true;
      ...
      translatedGsk.forEach(p => {
        if (p.x < 12.0) {
          footPointsCount++;
          if (p.x < 7.0 || p.y < 33.0 || p.y > 39.0) {
            footInPocket = false;
          }
        }
        if (isPointInPolygon(p, doorFrm)) {
          insideSashCount++;
        }
      });
      
      if (footInPocket && footPointsCount > 0) {
        results.push({ dx, dy, insideSashCount });
      }
    }
  }
  
  console.log(`Found ${results.length} valid pocket candidates.`);
  if (results.length === 0) {
    console.log("No candidates found that fit the pocket box!");
    return;
  }
  
  // Sort by minimum insideSashCount
  results.sort((a, b) => a.insideSashCount - b.insideSashCount);
  console.log("=== TOP 5 CANDIDATES ===");
  results.slice(0, 5).forEach((c, idx) => {
    console.log(`[${idx}] dx=${c.dx.toFixed(4)}, dy=${c.dy.toFixed(4)} -> insideSash=${c.insideSashCount}`);
  });
}
