...
function main() {
  ...
  // We sweep a broader range of dx (8940 to 9020) and dy (21250 to 21350)
  // Let's do a fast coarse sweep first
  console.log("Running coarse search...");
  const coarseStep = 1.0;
  const candidates = [];
  for (let dx = 8930; dx <= 9020; dx += coarseStep) {
    for (let dy = 21250; dy <= 21350; dy += coarseStep) {
      const tx = gskX + dx;
      const ty = gskY + dy;
      
      const translatedGsk = misplacedGsk.map(p => ({ x: p.x + dx, y: p.y + dy }));
      
      // Let's relax containment: we check if it is outside the solid parts of the profile
      // For now, let's just see if the gasket center falls in the interior region near the sash rebate.
      // Sash rebate is at X=[5, 25], Y=[30, 60]
      if (tx >= sashMinX - 5 && tx <= sashMinX + 25 && ty >= sashMinY - 5 && ty <= sashMinY + 45) {
        // Compute distance to sash
        let minD = Infinity;
        let insideSash = 0;
        translatedGsk.forEach(gp => {
          if (isPointInPolygon(gp, doorFrm)) insideSash++;
          doorFrm.forEach(sp => {
            const d = dist(gp, sp);
            if (d < minD) minD = d;
          });
        ...
        let insideFrame = 0;
        translatedGsk.forEach(gp => {
          if (isPointInPolygon(gp, mainFrm)) insideFrame++;
        });

        // We want minimal intersection (e.g. at most 3 points inside sash due to tight seal representation)
        if (insideSash <= 3 && insideFrame === 0) {
          candidates.push({ dx, dy, minD, insideSash, tx, ty });
        }
      }
    }
  }
  ...
  candidates.sort((a, b) => a.minD - b.minD || a.insideSash - b.insideSash);
  const bestCoarse = candidates[0];
  console.log(`Best coarse candidate: dx=${bestCoarse.dx.toFixed(2)}, dy=${bestCoarse.dy.toFixed(2)} -> center=(${bestCoarse.tx.toFixed(2)}, ${bestCoarse.ty.toFixed(2)}) minD=${bestCoarse.minD.toFixed(3)} insideSash=${bestCoarse.insideSash}`);

  console.log("Running fine search...");
  const fineStep = 0.05;
  const fineCandidates = [];
  for (let dx = bestCoarse.dx - 2.0; dx <= bestCoarse.dx + 2.0; dx += fineStep) {
    for (let dy = bestCoarse.dy - 2.0; dy <= bestCoarse.dy + 2.0; dy += fineStep) {
      const tx = gskX + dx;
      const ty = gskY + dy;
      const translatedGsk = misplacedGsk.map(p => ({ x: p.x + dx, y: p.y + dy }));
      ...
      if (insideSash <= 3 && insideFrame === 0) {
        ...
        fineCandidates.push({ dx, dy, minD, insideSash, tx, ty });
      }
    }
  }

  fineCandidates.sort((a, b) => a.minD - b.minD || a.insideSash - b.insideSash);
  console.log("=== TOP 5 FINE CANDIDATES ===");
  fineCandidates.slice(0, 5).forEach((c, idx) => {
    console.log(`[${idx}] dx = ${c.dx.toFixed(4)}, dy = ${c.dy.toFixed(4)} -> center=(${c.tx.toFixed(4)}, ${c.ty.toFixed(4)}) minD=${c.minD.toFixed(5)} insideSash=${c.insideSash}`);
  });
}
