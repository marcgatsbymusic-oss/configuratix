...
function main() {
  ...
  const dx = 8957.80;
  console.log(`Testing dx = ${dx} for various dy values...`);

  for (let dy = 21288; dy <= 21295; dy += 0.2) {
    ...
    console.log(`dy=${dy.toFixed(2)} -> X=[${minX.toFixed(2)}, ${maxX.toFixed(2)}] Y=[${minY.toFixed(2)}, ${maxY.toFixed(2)}] insideSash=${insideSash} insideFrame=${insideFrame}`);
  }
}
