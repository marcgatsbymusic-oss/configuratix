...
console.log(`Total segments in layer "${targetLayer}": ${segments.length}`);

// Test tolerances
const tols = [0.05, 0.1, 0.2, 0.5, 1.0, 1.5];
tols.forEach(tol => {
  const chains = chainSegments(segments, tol);
  const closed = chains.filter(c => dist(c[0], c[c.length-1]) <= tol);
  console.log(`Tolerance ${tol}mm: totalChains=${chains.length}, closedChains=${closed.length}`);
});
