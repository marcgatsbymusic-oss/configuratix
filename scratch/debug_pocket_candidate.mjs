...
function main() {
  ...
  const dx = 8956.5, dy = 21292.5;
  console.log(`Testing dx=${dx}, dy=${dy}...`);
  ...
  closed.forEach((p, idx) => {
    ...
    const wy = p.y + dy;
    if (wx < 12.0) {
      if (wx < 7.0 || wy < 33.0 || wy > 39.0) {
        failedCount++;
        if (failedCount <= 10) {
          console.log(`Point [${idx}] local=(${p.x.toFixed(4)}, ${p.y.toFixed(4)}) -> world=(${wx.toFixed(4)}, ${wy.toFixed(4)}) fails pocket bounds`);
        }
      }
    }
  });
  console.log(`Total failed points in pocket: ${failedCount} out of ${closed.length}`);
}
