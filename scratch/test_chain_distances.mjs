...
items.forEach(({ entity, tx }) => {
  ...
  if (pts && s.x >= thresholdX && e.x >= thresholdX) {
    segments.push({ start: s, end: e, pts });
  }
});

const chains = chainSegments(segments, 1.5);
console.log(`Chains built at 1.5mm: ${chains.length}`);
chains.forEach((c, idx) => {
  const gap = dist(c[0], c[c.length-1]);
  console.log(`  Chain ${idx}: size=${c.length}, gap=${gap.toFixed(4)}mm`);
});
