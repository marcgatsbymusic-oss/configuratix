...
const OUT_JSON = "c:/Users/Shadow/.gemini/antigravity/scratch/fantastic-octo-giggle/src/data/profiles/IgloEdge/IGE_WINDOW_MOVABLE_POST.json";
const SVG_OUT = "c:/Users/Shadow/.gemini/antigravity/scratch/fantastic-octo-giggle/public/IGE_MOVABLE_POST_LEFT_OPENING_FULL_COLORED.svg";
...
async function main() {
  console.log(`📂 Reading Cleaned DXF: ${INPUT_FILE}`);
  ...
  // Reconstruct interior sash gaskets (GSK_SSH_INT) from U-002 inserts
  const uniqueU002 = [];
  ...
  const rawContours = {
    ...
    // Glass (Left and Right) - Fixed: Loop over each contour individually to avoid merged glass pane
  ...
  console.log(`✅ Saved JSON to: ${OUT_JSON}`);

  // Build a beautiful SVG file (AutoCAD/Colored style)
  const scale = 8;
  const PAD = 20;
...
<svg xmlns="http://www.w3.org/2000/svg" width="${svgW}" height="${svgH}" style="background:#0d1117;">
  ...
  <defs>
    <pattern id="grid" width="${10*scale}" height="${10*scale}" patternUnits="userSpaceOnUse">
      <path d="M ${10*scale} 0 L 0 0 0 ${10*scale}" fill="none" stroke="#1a2233" stroke-width="0.5"/>
    </pattern>
  </defs>
  <rect width="100%" height="100%" fill="url(#grid)" />
  
  <!-- Left Sash (Split Horizontally) -->
  <path d="${toPath(output.layers.L_SSH_EXT.contours[0].points)}" fill="#e05a47" fill-opacity="1" stroke="#c04030" stroke-width="1.5" />
  <path d="${toPath(output.layers.L_SSH_INT.contours[0].points)}" fill="#4a90e2" fill-opacity="1" stroke="#2a70c2" stroke-width="1.5" />
  
  <!-- Right Sash (Split Horizontally) -->
  <path d="${toPath(output.layers.SSH_EXT.contours[0].points)}" fill="#e05a47" fill-opacity="1" stroke="#c04030" stroke-width="1.5" />
  <path d="${toPath(output.layers.SSH_INT.contours[0].points)}" fill="#4a90e2" fill-opacity="1" stroke="#2a70c2" stroke-width="1.5" />
  
  <!-- Movable Post (Split Horizontally) -->
  <path d="${toPath(output.layers.PST_EXT.contours[0].points)}" fill="#c59b27" fill-opacity="1" stroke="#a07c10" stroke-width="1.5" />
  <path d="${toPath(output.layers.PST_INT.contours[0].points)}" fill="#2ca02c" fill-opacity="1" stroke="#1a801a" stroke-width="1.5" />
`;

  // Colors mapping for all other elements
  const layerColors = {
    // Beads
    'L_BZD': '#e0a050',
    'BZD': '#e0a050',
    
    // EPDM Gaskets (Pink/Magenta/Purple/Cyan)
    ...
    'GSK_PST_L': '#cc44ff',
    'GSK_PST_R': '#cc44ff',
    'IGE_GSK_MD_MOVABLE_POST': '#9b30ff',
    'L_GSK_BZD': '#00d0d0',
    'GSK_BZD': '#00d0d0',
  ...
  const strokeColors = {
    'L_BZD': '#b07828',
    'BZD': '#b07828',
    'L_GSK_SSH_EXT': '#cc0055',
    'GSK_SSH_EXT': '#cc0055',
    'L_GSK_SSH_INT': '#cc0055',
    'GSK_SSH_INT': '#cc0055',
    'GSK_PST_L': '#9900cc',
    'GSK_PST_R': '#9900cc',
    'IGE_GSK_MD_MOVABLE_POST': '#6600cc',
    'L_GSK_BZD': '#008888',
    'GSK_BZD': '#008888',
    'L_SPACER': '#4a5a6a',
    'SPACER': '#4a5a6a',
  ...
  // Draw other layers (Beads, Gaskets, Spacers, Steel, Glass)
  const drawOrder = [
    'L_BZD', 'BZD',
    'L_GSK_SSH_EXT', 'GSK_SSH_EXT',
    'L_GSK_SSH_INT', 'GSK_SSH_INT',
    'GSK_PST_L', 'GSK_PST_R',
    'IGE_GSK_MD_MOVABLE_POST',
    'L_GSK_BZD', 'GSK_BZD',
    'L_SPACER', 'SPACER',
    'L_STEEL', 'STEEL',
    'L_GLS_EXT', 'L_GLS_MD', 'L_GLS_INT',
  ...
  drawOrder.forEach(layerName => {
    const layerData = output.layers[layerName];
    if (!layerData || !layerData.contours) return;

    const fillVal = layerColors[layerName] || '#888';
    const strokeVal = strokeColors[layerName] || '#555';
    const fillOpacity = layerName.includes('GLS') ? '0.5' : '1';
    const strokeWidth = layerName.includes('GSK') || layerName === 'BZD' || layerName === 'L_BZD' ? '1.5' : '1.0';

    layerData.contours.forEach(c => {
      svg += `  <path d="${toPath(c.points)}" fill="${fillVal}" fill-opacity="${fillOpacity}" stroke="${strokeVal}" stroke-width="${strokeWidth}" />\n`;
    });
  });
  ...
  console.log(`✅ Saved Colored SVG to: ${SVG_OUT}`);
}
