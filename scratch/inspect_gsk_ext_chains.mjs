...
try {
  ...
  console.log(`Total segments in IGE_GSK_EXT: ${segments.length}`);
  ...
  chains.forEach((c, idx) => {
    ...
    console.log(`Chain ${idx}: length=${c.length}, centroid=[${cx.toFixed(2)}, ${cy.toFixed(2)}], gap=${gap.toFixed(4)} mm`);
  });
