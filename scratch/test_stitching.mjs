...
function stitchChains(chains, tol = 1.5) {
  let currentChains = chains.map(pts => ({
    pts: [...pts],
  ...
  while (changed) {
    ...
    // Look for the closest pair of open endpoints
...
console.log('Stitched chains:');
stitched.forEach((c, idx) => {
  const gap = dist(c.pts[0], c.pts[c.pts.length - 1]);
  console.log(`  Chain ${idx}: size=${c.pts.length}, closed=${c.closed}, gap=${gap.toFixed(4)}mm`);
});
