...
try {
  ...
  console.log(`Found ${contours.length} contours on IGE_GSK_EXT:`);
  contours.forEach((c, idx) => {
    ...
    console.log(`Contour ${idx}: Centroid [${cx.toFixed(4)}, ${cy.toFixed(4)}], Bounding Box: X [${minX.toFixed(4)} to ${maxX.toFixed(4)}], Y [${minY.toFixed(4)} to ${maxY.toFixed(4)}]`);
  });
} catch (err) {
  console.error(err);
}
