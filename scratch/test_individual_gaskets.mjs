...
function main() {
  ...
  const options = [
    { dx: 8962.85, dy: 21300.10, name: "Current (wrong)" },
    { dx: 8956.76, dy: 21292.97, name: "Candidate B" },
    { dx: 8956.10, dy: 21292.80, name: "Candidate C" },
    { dx: 8955.50, dy: 21293.20, name: "Candidate D" }
  ];
  ...
  options.forEach(opt => {
    ...
    console.log(`\n=== ${opt.name} ===`);
    console.log(`dx=${opt.dx.toFixed(2)}, dy=${opt.dy.toFixed(2)}`);
    console.log(`Bounds: X=[${minX.toFixed(2)}, ${maxX.toFixed(2)}], Y=[${minY.toFixed(2)}, ${maxY.toFixed(2)}]`);
    console.log(`insideSash=${insideSash}, insideFrame=${insideFrame}, minD=${minD.toFixed(3)}`);
  });
}
