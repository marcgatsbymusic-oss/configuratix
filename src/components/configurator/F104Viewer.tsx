/**
 * F104Viewer.tsx
 * Parametric 3D viewer for the 1600-IGLO EDGE F104 Sash and Frame Window profile.
...
const getHandleHeight = (hMm: number): number => {
  const isDoorOrBalcony = false; // F104 is a standard window typology
  if (isDoorOrBalcony || hMm > 1800) return 1050;
...
function F104Assembly({
  ...
  // Dynamic handle height calculation based on GEMINI.md rule
  ...
  useEffect(() => {
    ...
    // Dynamic tilt angle calculation using Math.asin(150 / height) as mandated by GEMINI.md
  ...
  }, [windowState, clock, isAuto, heightMm]);
...
export interface F104ViewerProps {
  width?: number;
  height?: number;
  colorExt?: string;
  colorInt?: string;
...
export const F104Viewer: React.FC<F104ViewerProps> = ({
  width = 1000,
  height = 1000,
  colorExt = '#e8e0d4',
  colorInt = '#f0ece6',
  ...
  const minW = activeLimits?.minWidth  || 210; // F104 min width = 210
  const maxW = activeLimits?.maxWidth  || 1576; // F104 max width = 1576
  const minH = activeLimits?.minHeight || 210; // F104 min height = 210
  const maxH = activeLimits?.maxHeight || 3078; // F104 max height = 3078
  ...
  const frameDepthMm = 82.0; // IGLO EDGE depth is 82mm
  const targetX = W_M / 2;
  const targetY = H_M / 2;
