 ...
 * Built using IGE_DW_PST_LEFT_TT-T.json:
 ...
 * - PST Trim rules: PST_EXT is trimmed 75 mm shorter on top and bottom (150 mm total); PST_INT is trimmed 48 mm shorter on top and bottom (96 mm total).
 */
...
const transformMovableData = (raw: any): ProfileData => {
  ...
  const toMirror = ['SSH_INT', 'GSK_SSH_INT'];
...
function F202RV3Assembly({
  ...
  // Asymmetric glass pane rendering for passive sash
  ...
  const renderRightSashHorizontalSide = (
    ...
    // Shortening values (in mm) relative to the stulp side
    const shExtGsk  = 142.32;
    const shIntGsk  = 124.42;
    const shSpacer  = 141.5;
    const shBzd     = 126.26;

    // Helper to get parameters depending on whether it is bottom rail (stulp at z=0) or top rail (stulp at z=len)
  ...
  // Render Movable Post profiles (Square butt joint cut, trimmed per IGE_DW_PST_LEFT_TT-T.json)
  const renderMovablePost = (sashHeightMm: number) => {
    // PST_EXT (exterior): trimmed 75 mm shorter on both top and bottom (150 mm total)
    const extLenMm = sashHeightMm - 150;
    const extPos: [number, number, number] = [0, 0, 75 * scale];

    // PST_INT (interior): trimmed 48 mm shorter on both top and bottom (96 mm total)
  ...
  // Dimensions & Overlap calculations (Based on split centerline and splitRatio)
  const splitX = widthMm * splitRatio;
  const Ww_left = (splitX + 54) * scale;  // Left sash width: splitX + 54mm overlap
  const Ww_right = ((widthMm - splitX) + 54) * scale; // Right sash width: (width - splitX) + 54mm overlap
  const W_sash = Ww_right - 108 * scale;
  const rightSashX = W - Ww_right + 108 * scale; // Shifted right sash group by 108mm overlap
  ...
  const rightPivotX = W_sash - 50 * scale; // Pivot 50mm from the right stile in local coordinates
        ...
        <group position={[rightPivotX, rightPivotY, rightPivotZ]} name="rightPivotGroup">
          <group ref={rightSashPivotRef} name="rightSashPivot">
            <group position={[-rightPivotX, -rightPivotY, -rightPivotZ]}>
              {/* Bottom rail */}
              <group rotation={[0, 0, 0]}><group rotation={[0, Math.PI / 2, 0]}>{renderRightSashHorizontalSide(W_sash / scale, 1, 108, false)}</group></group>
              {/* Right stile (meets frame, positioned at W_sash) */}
              <group position={[W_sash, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
                <group rotation={[0, Math.PI / 2, 0]}>
                  {renderStandardSashSide(heightMm, -1, Ww_right / scale)}
                </group>
              </group>
              {/* Top rail */}
              <group position={[W_sash, H, 0]} rotation={[0, 0, Math.PI]}>
                <group rotation={[0, Math.PI / 2, 0]}>
                  {renderRightSashHorizontalSide(W_sash / scale, 1, (W_sash / scale) - heightMm, true)}
                </group>
              </group>
              {/* Left meeting stile (Sourced from transformed Movable Post, positioned at 0, runs down) */}
              ...
              {/* Glass Pane */}
              {renderAsymmetricGlassPane(W_sash / scale, heightMm, leftGlsExt, 137, 99)}
              {renderAsymmetricGlassPane(W_sash / scale, heightMm, leftGlsInt, 137, 99)}

              {/* Click hotspot to trigger Inactive Right Sash state changes (positioned 80mm from the meeting stile) */}
              {!isColorPaletteOpen && (
              ...
              {/* Large R Indicator (centered over the right sash glass pane) */}
              <Html position={[W_sash / 2, heightMm * scale / 2, 5 * scale]} center>
                <div style={{
                  fontSize: '96px',
                  color: 'rgba(234, 182, 118, 0.85)',
                  textShadow: '0 0 16px rgba(0, 0, 0, 0.9)',
...
export interface F202RV3ViewerProps {
  width?: number;
  height?: number;
  splitRatio?: number;
  colorExt?: string;
...
export const F202RV3Viewer: React.FC<F202RV3ViewerProps> = ({
  width = 1350,
  height = 1245,
  splitRatio = 0.5,
  colorExt = '#e8e0d4',
