/**
 * F202LViewer.tsx
 * Parametric 3D viewer for the IGE_F202L Double Sash Window profile system (Iglo Edge series).
 * Sourced from IGE_F104.json (Left Sash & Frame) and IGE_MOVABLEPOST_LEFT_OPENING.json (Right Sash & Movable Post).
 */
...
function F202LAssembly({
  ...
  // 2. Left Sash Profiles (Sourced from F104)
  ...
  // Render left sash segments (Sourced from F104)
  const renderLeftSashSide = (len: number, uSign: number, uOff: number) => (<>
    {leftSshExt.map((c, i) => (
      <FrameSegment key={`lSshExt_${i}`} layerName="SSH_EXT" scaleFactor={scale}
        length={len} vertices={c} matType="ext"
        color={colorExt} textureUrl={colorExtTexture}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
      />
    ...
    {leftSshInt.map((c, i) => (
      <FrameSegment key={`lSshInt_${i}`} layerName="SSH_INT" scaleFactor={scale}
        length={len} vertices={c} matType="int"
        color={colorInt} textureUrl={colorIntTexture}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
      />
    ...
    {leftBzd.map((c, i) => (
      <FrameSegment key={`lBzd_${i}`} layerName="BZD" scaleFactor={scale}
        length={len} vertices={c} matType="int"
        color={colorInt} textureUrl={colorIntTexture}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
        uvMode="rail"
    ...
    {leftGskBzd.map((c, i) => (
      <FrameSegment key={`lGskBzd_${i}`} layerName="GSK_BZD" scaleFactor={scale}
        length={len} vertices={c} matType="gsk"
        color={colorGsk}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
      />
    ...
    {leftGskSshInt.map((c, i) => (
      <FrameSegment key={`lGskSI_${i}`} layerName="GSK_SSH_INT" scaleFactor={scale}
        length={len} vertices={c} matType="gsk"
        color={colorGsk}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
      />
    ...
    {leftSpacer.map((c, i) => (
      <FrameSegment key={`lSpacer_${i}`} layerName="SPACER" scaleFactor={scale}
    ...
    {leftGskSshExt.map((c, i) => (
      <FrameSegment key={`lGskSE_${i}`} layerName="GSK_SSH_EXT" scaleFactor={scale}
        length={len} vertices={c} matType="gsk"
        color={colorGsk}
        origin={commonOrigin} uSign={uSign} uOffset={uOff}
      />
  ...
  // Render right sash standard segments (Sourced from F104)
  const renderRightSashSideF104 = (len: number, uSign: number, uOff: number) => (<>
    {leftSshExt.map((c, i) => (
  ...
  // Handle position on active (right) sash meeting stile
  ...
  const handleX = 83 * scale; // Placed on left meeting stile of right sash
  ...
  // Update Left Sash Animation Targets (Opens towards camera, negative Z direction)
  useEffect(() => {
    ...
    s.duration = leftState === 'open_side' ? 2.4 : 1.2; // Double time when opening
  }, [leftState, clock]);

  // Update Right Sash Animation Targets (Opens towards camera, negative Z direction)
  useEffect(() => {
    ...
    s.targetTilt = rightState === 'open_tilt' ? -Math.asin(150 / heightMm) : 0; // Negative rotation to tilt towards camera
    s.startHandle = currentHandle.current;
    s.targetHandle = rightState === 'open_side' ? -Math.PI / 2 : (rightState === 'open_tilt' ? -Math.PI : 0);
    s.startTime = clock.getElapsedTime();
    s.duration = rightState !== 'closed' ? 2.4 : 1.2; // Double time when opening
  ...
  return (
    <group ref={setGroupObj}>
      ...
      {/* ── Inactive Left Sash + Movable Post Block (Opens Side Swing only, no handle) ── */}
        ...
        <group position={[leftPivotX, leftPivotY, leftPivotZ]} name="leftPivotGroup">
          <group ref={leftSashPivotRef} name="leftSashPivot">
            <group position={[-leftPivotX, -leftPivotY, -leftPivotZ]}>
              {/* Bottom rail (Sourced from F104) */}
              <group rotation={[0, 0, 0]}><group rotation={[0, Math.PI / 2, 0]}>{renderLeftSashSide(SW + 54, 1, 0)}</group></group>
              {/* Right meeting stile (Sourced from Movable Post) */}
              <group position={[Ww_left, 0, 0]} rotation={[0, 0, Math.PI / 2]}>
                <group rotation={[0, Math.PI / 2, 0]}>
                  {renderRightSashMeetingStile(heightMm, -1, Ww_left)}
                  {/* Movable Post - grouped together inside the left sash block */}
                  {renderMovablePost(heightMm)}
                </group>
              </group>
              {/* Top rail (Sourced from F104) */}
              <group position={[Ww_left, H, 0]} rotation={[0, 0, Math.PI]}><group rotation={[0, Math.PI / 2, 0]}>{renderLeftSashSide(SW + 54, 1, Ww_left - H)}</group></group>
              {/* Left stile (meets frame, Sourced from F104) */}
              <group position={[0, H, 0]} rotation={[0, 0, -Math.PI / 2]}><group rotation={[0, Math.PI / 2, 0]}>{renderLeftSashSide(heightMm, -1, Ww_left - H)}</group></group>
        ...
        <group position={[rightPivotX, rightPivotY, rightPivotZ]} name="rightPivotGroup">
          <group ref={rightSashPivotRef} name="rightSashPivot">
            <group position={[-rightPivotX, -rightPivotY, -rightPivotZ]}>
              {/* Bottom rail */}
              <group rotation={[0, 0, 0]}><group rotation={[0, Math.PI / 2, 0]}>{renderRightSashSideF104(SW + 54, 1, 0)}</group></group>
              {/* Right stile */}
              <group position={[Ww_right, 0, 0]} rotation={[0, 0, Math.PI / 2]}><group rotation={[0, Math.PI / 2, 0]}>{renderRightSashSideF104(heightMm, -1, Ww_right)}</group></group>
              {/* Top rail */}
              <group position={[Ww_right, H, 0]} rotation={[0, 0, Math.PI]}><group rotation={[0, Math.PI / 2, 0]}>{renderRightSashSideF104(SW + 54, 1, Ww_right - H)}</group></group>
              {/* Left meeting stile */}
              <group position={[0, H, 0]} rotation={[0, 0, -Math.PI / 2]}><group rotation={[0, Math.PI / 2, 0]}>{renderRightSashSideF104(heightMm, -1, Ww_right - H)}</group></group>
...
export interface F202LViewerProps {
  width?: number;
  height?: number;
  colorExt?: string;
  colorInt?: string;
...
export const F202LViewer: React.FC<F202LViewerProps> = ({
  width = 1200,
  height = 1200,
  colorExt = '#e8e0d4',
  colorInt = '#f0ece6',
  ...
  const handleLeftInteraction = useCallback((state: SashState) => {
    if (state === 'open_side' && rightState !== 'open_side') {
      return; // Only allow opening left (inactive) if right (active) is already open_side
    }
    setLeftState(state);
    setAutoRotate(false);
  }, [rightState]);
  ...
  const handleRightInteraction = useCallback((state: SashState) => {
    if (state === 'closed' || state === 'open_tilt') {
      setLeftState('closed'); // Auto-close left (inactive) sash if right (active) sash is closed or tilted
    }
    setRightState(state);
    setAutoRotate(false);
  }, []);
  ...
  return (
    <div className="absolute inset-0" style={{ background: '#dde4ed' }}>
      ...
      >
        Double Sash Movable Post (Active Right)
      </div>
