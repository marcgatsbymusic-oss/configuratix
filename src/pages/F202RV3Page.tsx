/**
 * F202RV3Page.tsx
 * Full-screen standalone test page for the IGE_DW_PST_LEFT_TT-T double window with movable post (Iglo Edge series).
 * Route: /f202rv3
 *
 * Left sash: active (tilt & turn, clockwise handle).
 * Right sash: passive (turn only, no handle, carries movable post).
 * Sequence lock: right can only open after left is open_side.
 */

import React, { useState } from 'react';
import { F202RV3Viewer } from '../components/configurator/F202RV3Viewer';
...
export const F202RV3Page: React.FC = () => {
    ...
    <div className="fixed inset-0 flex overflow-hidden" style={{ background: '#0a0a14' }}>
      ...
      <div className="relative flex-1">
        ...
        >
          ...
          F202RV3 — Parametric Double Window (V3 Trim Rules)
        </div>
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            <span>Right sash — turn only (no handle)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-[#eab676]" />
            <span>Movable Post — swings with right sash</span>
          </div>
          <div className="mt-1 text-white/40 border-t border-white/5 pt-1">
            Click hotspot to toggle right sash or left sash (active) open/close
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div style={{ marginTop: 'auto', paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="text-[9px] leading-relaxed" style={{ color: 'rgba(255,255,255,0.2)' }}>
            IGE_DW_PST_LEFT_TT-T Rule Config<br />
            Left Active Sash (T&amp;T) · Right Inactive Sash (Turn)<br />
            PST_EXT shortening −75mm top+bot<br />
            PST_INT shortening −48mm top+bot<br />
            Inner-sash design gap = 8mm
          </div>
        </div>
      </div>
    </div>
